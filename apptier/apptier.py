...
def image_classification(img_encode):
    ...
    if object_name_test is None:
        object_name_test = os.path.basename("imageToSave.jpg")
    try:
        response = upload_file(object_name_test, BUCKET_NAME_INPUT, '{}{}'.format(S3_INPUT_FOLDER,"imageToSave.jpg"))
...
# 4) store result in s3 and put in response queue
        file_to_store = '{}_Result.txt'.format(job_id)
        s3_writetofile = open(file_to_store, "w+")
        s3_writetofile.write(response_queue_message)
        s3_writetofile.close()

        upload_file(file_to_store, BUCKET_NAME_OUTPUT, '{}{}'.format(S3_OUTPUT_FOLDER, file_to_store))
        print('uploaded file to bucket')
